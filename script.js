...
startAttendanceBtn.addEventListener('click', () => {
    attendanceSessionKey = Math.floor(100000 + Math.random() * 900000).toString();
    registeredUsers.forEach(user => user.attendance = 'Absent');
    qrCodeImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${attendanceSessionKey}`;
    qrCodeText.textContent = attendanceSessionKey;
...
attendanceForm.addEventListener('submit', (e) => {
    ...
    if (enteredCode === attendanceSessionKey) {
